...
const prepareListingPayment = async (req, res) => {
  try {
    ...
    if (listingId) {
      ...
      console.log('[DEBUG] preparing listing payment. Incoming listingData:', JSON.stringify(listingData));
      ...
      // Explicitly check metadata
      if (!newListingData.metadata) {
        newListingData.metadata = {};
      }

      listing = await Listing.create(newListingData);
      console.log('Created draft listing:', listing._id, 'with tx_ref:', tx_ref);
      console.log('[DEBUG] Saved listing metadata:', listing.metadata);
    }
